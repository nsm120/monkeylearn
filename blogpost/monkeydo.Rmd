...
I'd seen in the package docs and on the [MonkeyLearn FAQ](http://help.monkeylearn.com/frequently-asked-questions/queries/can-i-classify-or-extract-more-than-one-text-with-one-api-request) that batching up to 200 texts was possible[^2]. So, I decide to first look into the mechanics of how text batching is done.
...
I forked the package repo and set about rummaging through the source code. Blissfully, everything is nicely commented and the code was quite readable. 

Inside `monkeylearn_classify()` I spot a call to `monkeylearn_parse()` which I suspect is a utility function. I find it in `utils.R`.
...
My work on the monkeylearn package so far has been rewarding to say the least. All the credit goes to Maëlle, who's provided guidance in at least three languages, English, [French](https://twitter.com/ma_salmon/status/971992354763649024), and R. I couldn't be more stoked for future collaborations -- on y va!
