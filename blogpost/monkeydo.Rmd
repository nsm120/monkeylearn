...
It was clear that even classifying my relatively small data was going to take a looong time `r emo::ji("see_no_evil")`. I updated the function to write each row out to an RDS file after it was classified inside the loop (with an addition along the lines of `write_rds(out[i, ], glue::glue("some_directory/{i}.rds"))`) so that I wouldn't have to rely on the funciton successfully finishing execution in one run. Still, I didn't like my options.
...
Maëlle's been a fantastic mentor through and through, providing guidance in at least three languages -- English, [French](https://twitter.com/ma_salmon/status/971992354763649024), and R, despite the time difference and `r emo::ji("baby")`(!). I couldn't be more stoked for future collaborations. *On y va*!
